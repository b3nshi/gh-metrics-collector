/**
 * This script is used to process the data collected by the metrics-collector.js script.
 * It is used to calculate the metrics and the data is used to display the dashboard.
 * It is also used to calculate the throughput balance.
 * The goal is to make the data available for the dashboard.html script to use.
 */
const fs = require("fs");
...
function processMetrics() {
  if (!fs.existsSync(DATA_FILE))
    return console.error(`❌ No ${DATA_FILE} found.`);
  ...
  const mergers = Object.entries(
    ...
    avgTime:
      prs
        ...
        .reduce((a, b) => a + b.mergeTime, 0) / merged,
  }));

  const reviewers = Object.entries(
    prs.reduce((acc, p) => {
      (p.reviews || []).forEach((r) => (acc[r] = (acc[r] || 0) + 1));
      return acc;
    }, {})
