...
```Bash
node index.js -t YOUR_TOKEN -r owner/repo --from yyyy-mm-dd --until yyyy-mm-dd
```
...
> [!NOTE]
> If the script is interrupted by GitHub's Rate Limiter, it will save a `.pending_state.json`. Simply run the collector command again to resume exactly where it left off.
...
3.  **Implement:**
    * Add raw data points in `index.js`.
    * Update mathematical processing in `processor.js`.
